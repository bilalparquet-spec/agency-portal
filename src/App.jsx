...
const AGENCIES = [
  { id:1, ar:"وكالة الجزائر العاصمة", fr:"Agence Alger Centre", city:{ar:"الجزائر",fr:"Alger"}, wilaya:"16", initials:"وج", rating:4.9,  trips:450, exp:8, verified:true,  phone:"+213 555 12 34 56", about:{ar:"وكالة رائدة في تأجير السيارات بالجزائر العاصمة منذ 2016. نوفر مجموعة واسعة من السيارات الفاخرة والعائلية.",fr:"Agence leader de location à Alger depuis 2016.",en:"Leading car rental agency in Algiers since 2016."},
    plan:"premium", monthlyPrice:8000, subscriptionStart:"2025-01-15", subscriptionEnd:"2026-01-15", subscriptionMonths:12, renewalRequest:false },
  { id:2, ar:"وهران درايف",           fr:"Oran Drive",          city:{ar:"وهران",fr:"Oran"},   wilaya:"31", initials:"ود", rating:4.8,  trips:312, exp:5, verified:true,  phone:"+213 555 98 76 54", about:{ar:"أفضل خدمة تأجير في وهران بأسعار تنافسية وسيارات حديثة.",fr:"Meilleure location à Oran à prix compétitifs.",en:"Best car rental in Oran with competitive prices."},
    plan:"standard", monthlyPrice:5000, subscriptionStart:"2025-03-01", subscriptionEnd:"2025-09-01", subscriptionMonths:6, renewalRequest:false },
  { id:3, ar:"قسنطينة بريميوم",       fr:"Constantine Premium",  city:{ar:"قسنطينة",fr:"Constantine"}, wilaya:"25", initials:"قب", rating:4.85, trips:280, exp:6, verified:true,  phone:"+213 555 44 55 66", about:{ar:"سيارات فاخرة وخدمة ممتازة في مدينة الجسور المعلقة.",fr:"Voitures de luxe à Constantine.",en:"Luxury cars in the City of Bridges."},
    plan:"standard", monthlyPrice:5000, subscriptionStart:"2025-04-01", subscriptionEnd:"2025-10-01", subscriptionMonths:6, renewalRequest:false },
  { id:4, ar:"عنابة للسيارات",        fr:"Annaba Cars",          city:{ar:"عنابة",fr:"Annaba"}, wilaya:"23", initials:"عس", rating:4.7,  trips:195, exp:4, verified:false, phone:"+213 555 77 88 99", about:{ar:"خدمة موثوقة في عنابة وضواحيها.",fr:"Service fiable à Annaba.",en:"Reliable service in Annaba."},
    plan:"basic", monthlyPrice:3500, subscriptionStart:"2025-02-20", subscriptionEnd:"2025-05-20", subscriptionMonths:3, renewalRequest:true },
  { id:5, ar:"سطيف أوتو",             fr:"Sétif Auto",           city:{ar:"سطيف",fr:"Sétif"},   wilaya:"19", initials:"سا", rating:4.75, trips:220, exp:3, verified:true,  phone:"+213 555 33 22 11", about:{ar:"الوكالة الأولى في سطيف والمنطقة الشرقية.",fr:"Première agence à Sétif.",en:"Top agency in Sétif."},
    plan:"standard", monthlyPrice:5000, subscriptionStart:"2025-05-01", subscriptionEnd:"2025-11-01", subscriptionMonths:6, renewalRequest:false },
  { id:6, ar:"تيزي وزو للأعراس",      fr:"Tizi Wedding Cars",    city:{ar:"تيزي وزو",fr:"Tizi Ouzou"}, wilaya:"15", initials:"تع", rating:4.95, trips:180, exp:7, verified:true,  phone:"+213 555 66 77 88", about:{ar:"متخصصون في سيارات الأعراس الفاخرة بتيزي وزو والمنطقة.",fr:"Spécialistes des voitures de mariage.",en:"Wedding car specialists in Tizi Ouzou."},
    plan:"premium", monthlyPrice:8000, subscriptionStart:"2025-01-01", subscriptionEnd:"2025-09-01", subscriptionMonths:8, renewalRequest:false },
];
...
const AGENCY_ACCOUNTS = [
  { agencyId:1, username:"alger.drive",   password:"alger2024",  status:"approved" },
  { agencyId:2, username:"oran.drive",    password:"oran2024",   status:"approved" },
  { agencyId:3, username:"constantine",   password:"cst2024",    status:"approved" },
  { agencyId:4, username:"annaba.cars",   password:"annaba2024", status:"approved" },
  { agencyId:5, username:"setif.auto",    password:"setif2024",  status:"approved" },
  { agencyId:6, username:"tizi.wedding",  password:"tizi2024",   status:"approved" },
];
...
export default function App() {
  ...
  const handleLogin = () => {
    const acc = AGENCY_ACCOUNTS.find(a => a.username === loginForm.username && a.password === loginForm.password);
    if (!acc) { setLoginErr(lang==="ar"?"اسم المستخدم أو كلمة المرور غير صحيحة":lang==="fr"?"Identifiants incorrects":"Invalid username or password"); return; }
    const ag = AGENCIES.find(a => a.id === acc.agencyId);
    if (!ag) { setLoginErr("Agency not found"); return; }
    setLoggedIn(ag);
    setLoginErr("");
  };
      ...
      <div style={{width:"100%",maxWidth:420,animation:"fadeUp .5s ease both"}}>
        ...
        <div style={{textAlign:"center",marginBottom:36}}>
          ...
          <div style={{fontSize:13,color:"rgba(255,255,255,.4)"}}>
            {lang==="ar"?"درايف RENT — منصة التأجير الجزائرية":"DriveRENT — Algeria's Car Rental Platform"}
          </div>
        </div>
        ...
        <div style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(124,58,237,.2)",borderRadius:20,padding:"32px 28px"}}>
          <div style={{marginBottom:20}}>
            ...
            <input
              ...
              placeholder={lang==="ar"?"مثال: alger.drive":"e.g. alger.drive"}
          ...
          {/* Demo accounts hint */}
          <div style={{marginTop:22,padding:"14px",background:"rgba(124,58,237,.07)",border:"1px solid rgba(124,58,237,.15)",borderRadius:12}}>
            <div style={{fontSize:10,color:"rgba(255,255,255,.35)",fontWeight:700,textTransform:"uppercase",letterSpacing:"1px",marginBottom:10}}>
              {lang==="ar"?"حسابات تجريبية":lang==="fr"?"Comptes démo":"Demo Accounts"}
            </div>
            {AGENCY_ACCOUNTS.slice(0,3).map(acc=>(
              <div key={acc.agencyId} onClick={()=>{setLoginForm({username:acc.username,password:acc.password});setLoginErr("");}} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,.05)",cursor:"pointer",fontSize:11}}>
                <span style={{color:"rgba(255,255,255,.6)",fontFamily:"monospace"}}>{acc.username}</span>
                <span style={{color:"rgba(124,58,237,.8)",fontFamily:"monospace"}}>{acc.password}</span>
              </div>
            ))}
            <div style={{fontSize:10,color:"rgba(255,255,255,.25)",marginTop:8,textAlign:"center"}}>
              {lang==="ar"?"انقر على حساب للملء التلقائي":"Click to auto-fill"}
            </div>
          </div>
        </div>

        <div style={{textAlign:"center",marginTop:20,fontSize:11,color:"rgba(255,255,255,.2)"}}>
          © 2025 DriveRENT — برمجة و تطوير غواط عبد الرحمان
        </div>
      </div>
    </div>
  );
